  ...
  public boolean exportAll(final Path file) {
    try (final PrintWriter printWriter = new PrintWriter(file.toFile())) {
      ...
      for (final Anime element : persistence.fetchAnimeList()) {
        writer.object().key("title").value(element.getTitle()).key("type").value(element.getTypeAsString()).key("episodes")
            .value(element.getEpisodes()).key("infoLink").value(element.getInfoLink()).key("location").value(element.getLocation())
            .endObject();
      }
      writer.endArray();
      writer.array();
      ...
      for (final WatchListEntry element : persistence.fetchWatchList()) {
        writer.object().key("thumbnail").value(element.getThumbnail()).key("title").value(element.getTitle()).key("infoLink")
            .value(element.getInfoLink()).endObject();
      }
