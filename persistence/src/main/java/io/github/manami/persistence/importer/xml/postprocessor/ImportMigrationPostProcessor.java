...
public class ImportMigrationPostProcessor {
  ...
  public ImportMigrationPostProcessor(final String currentToolVersion, final String documentVersion, final List<Anime> animeListEntries,
      final List<FilterEntry> filterListEntries, final List<WatchListEntry> watchListEntries) {
  ...
  private void migrateCdnUrl(final MinimalEntry anime) {
    ...
    final String thumbnail = anime.getThumbnail().toString();

    try {
      if (isNotBlank(thumbnail) && thumbnail.startsWith(oldCdnUrl)) {
        anime.setThumbnail(new URL(thumbnail.replaceAll(oldCdnUrl, newCdnUrl)));
      }
    } catch (MalformedURLException e) {
      log.error("Converting thumbnail URL for [{}] created an error", anime.getTitle(), e);
    }
  }
}
